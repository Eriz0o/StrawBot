 /*
* StrawBot es una creación de Eriz0o
* StrawBot no tiene ningun fin de lucro
* Eriz0o se reserva todos los derechos de autor
* © 2021 Eriz0o, INC.
...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:0000\n' // Nombre
            + 'ORG:Erizo;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=5493735482355:+54 9 3735 48-2355\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
/******ARCHIVOS ANTILINK POR ERIZ0******/
const antilink = JSON.parse(fs.readFileSync('./src/antilink.json'))

/******FIN ARCHIVOS ANTILINK POR ERIZ0******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'StrawBot'; if (!author) author = 'Erizo';	
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] Este comando solo puede ser utilizado por un admin del grupo ¯\_(ツ)_/¯',
					ownerB: '[❗] Este comando solo lo usa StrawBot',
					admin: '[❗] Este comando solo puede ser utilizado por administradores del grupo ¯\_(ツ)_/¯',
					...
					daftarB: `「y vos quien sos 🤨\n\nNo estas registrado en mi base de datos \n\nComando : ${prefix}daftar Nombre\nEjemplo : ${prefix}daftar erizo`,
				}
			}
    			const apakah = ['Si','No']
	       ...
	       case 'troleo':
               ...
               break*/
                 ...
                 case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recibido✅\n\nAgregando cargo administrativo:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
		...
		if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ya esta activada no molestes >:(')
						welkom.push(from)
